...
from src.rag.vectorstore.hybrid_index import init_sparse_embedder
...
_corpus_documents: list[RetrievedDocument] = []
_initialized = False
_embedding_warmed_up = False


def initialize_rag_components():
    global _sparse_embedder, _bm25, _reranker, _corpus_documents, _initialized
    ...
    if _initialized:
        ...
        return {
            ...
            "bm25": _bm25,
            "reranker": _reranker,
            "corpus_documents": _corpus_documents,
        }
    ...
    try:
        ...
        try:
            ...
            documents_texts = [doc.text for doc in documents]
            logger.info(f"Loaded {len(_corpus_documents)} documents from corpus")
...
def warmup_embedding_model() -> bool:
    global _embedding_warmed_up

    if _embedding_warmed_up:
        logger.info("Embedding model already warmed up")
        return True

    try:
        embed_texts(["warmup query"])
        _embedding_warmed_up = True
        logger.info("Embedding model warmed up successfully")
        return True
    except Exception as error:
        logger.warning("Failed to warm up embedding model: %s", error)
        return False
...
def get_corpus_documents() -> list[RetrievedDocument]:
...
def get_initialization_status() -> dict:
    return {
        ...
        "embedding_warmed_up": _embedding_warmed_up,
        "corpus_documents": len(_corpus_documents),
